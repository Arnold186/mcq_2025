...
const StudentLogin: React.FC = () => {
  ...
  const handleSubmit = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    // Simulate Login - Create a session profile
    const studentProfile = {
      name: formData.name,
      registrationNumber: "REG-" + Math.floor(1000 + Math.random() * 9000), // Random reg num for login simulation if known
      email: "student@example.com",
    };

    // Check if we have a stored profile to preserve the Reg Num if meaningful, else overwrite
    const existing = localStorage.getItem("studentProfile");
    if (existing) {
      const parsed = JSON.parse(existing);
      if (parsed.name === formData.name) {
        // Keep existing reg number
        studentProfile.registrationNumber = parsed.registrationNumber;
        studentProfile.email = parsed.email;
      }
    }

    localStorage.setItem("studentProfile", JSON.stringify(studentProfile));
    console.log("Logged in as:", studentProfile);
